function [obj, eidx] = train(X, Y, path2save, holdout)
    ...
    % randnegidx = randnegidx(1:size(ypos, 1) * 0.5);
    ...
    eidx = elasticfilter(trainx, trainy);
    disp('eidx:');
    disp(eidx);
    trainx = trainx(:, eidx);
    testx = testx(:, eidx);
    ...
    fprintf('Saving model to %s', path2save);
    save(path2save, 'obj');
end