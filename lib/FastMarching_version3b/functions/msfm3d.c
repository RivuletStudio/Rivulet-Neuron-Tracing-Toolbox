...
double CalculateDistance(double* T, double Fijk, int* dims, int i, int j, int k,
                         bool usesecond, bool usecross, bool* Frozen) {
  /* Loop variables */
  int q, t;
  ...
  double G1[18] = {1,
                   ...
                   0.3333333333333,
                   0.3333333333333};
  double G2[18] = {2.250, 2.250, 2.250, 2.250, 1.125, 1.125,
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  dims_sp[0] = dims_sp_c[0];
  dims_sp[1] = dims_sp_c[1];
  dims_sp[2] = dims_sp_c[2];
      ...
      if (isntfrozen3d(i, j, k, dims, Frozen)) {
        ...
        if (T[IJK_index] > 0) {
          ...
          if (neg_pos >= neg_free) {
            ...
            if (Ed) {
              neg_listo =
                  (double*)realloc(neg_listo, neg_free * sizeof(double));
            }
          }
